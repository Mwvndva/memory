...
void main() {
  ...
  group('Centralized Feed Refresh and Cache Invalidation Policy Tests', () {
    test(
      'User-initiated refresh always proceeds, system/automated requests are throttled (5s)',
      () async {
        ...
        expect(
          mockRepo.fetchCallCount,
          1,
          reason: 'System request within 5s must be throttled',
        );
        ...
        expect(
          mockRepo.fetchCallCount,
          2,
          reason: 'User force refresh must bypass throttling',
        );
      },
    );

    test(
      'Account switches clear state manager indices and memory states',
      () async {
        final sharedPreferences = await SharedPreferences.getInstance();
        final container = ProviderContainer(
          overrides: [
            ...
            memoryRepositoryProvider.overrideWith(
              (ref) => MockMemoryRepositoryForRefresh(ref),
            ),
          ],
        );
        addTearDown(container.dispose);
        ...
        expect(
          container.read(feedProvider).memories,
          isEmpty,
          reason: 'Feed state must clear upon logout',
        );
      },
    );
  });
