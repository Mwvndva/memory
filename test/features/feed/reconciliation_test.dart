...
class MockMemoryRepository extends MemoryRepository {
  ...
  Future<FeedPageResult> fetchFeed({String? cursor, int limit = 20}) async {
    ...
    if (cursor == 'page-2-cursor') {
      return const FeedPageResult(memories: [_carol], nextCursor: null);
    }
...
Future<({ProviderContainer container, FeedStateManager notifier})>
makeContainer() async {
...
void main() {
  ...
  group('Feed Reconciliation Engine Tests', () {
    // ── Scenario 1 ───────────────────────────────────────────────────────────
    test(
      '1. reconcile(replaceAll) adopts backend ordering with no duplicates',
      () async {
        ...
        expect(
          s2.memories.map((m) => m.id).toSet().length,
          2,
    ...
    // ── Scenario 2 ───────────────────────────────────────────────────────────
    test(
      '2. Deduplication uses canonical id, not the unstable time string',
      () async {
        final (:container, :notifier) = await makeContainer();
        addTearDown(container.dispose);
        ...
        final aliceInFeed = reconciled.firstWhere(
          (m) => m.id == 'uuid-alice-1',
        );
        expect(
          aliceInFeed.time,
    ...
    // ── Scenario 3 ───────────────────────────────────────────────────────────
    test(
      '3. reconcile(prepend) inserts new item at front; updates existing by id',
      () async {
        final (:container, :notifier) = await makeContainer();
        addTearDown(container.dispose);
        ...
        expect(s.memories.length, 3, reason: 'New item must be added');
        expect(
          s.memories.first.id,
          'uuid-carol-3',
        ...
        final carolInFeed = s2.memories.firstWhere(
          (m) => m.id == 'uuid-carol-3',
        );
        expect(
          carolInFeed.caption,
    ...
    // ── Scenario 4 ───────────────────────────────────────────────────────────
    test(
      '4. removeById removes exactly one item; index stays consistent',
      () async {
        final (:container, :notifier) = await makeContainer();
        addTearDown(container.dispose);
    ...
    test('5. Overlapping pagination pages produce no duplicates', () async {
      ...
      expect(
        reconciled.where((m) => m.id == 'uuid-bob-2').length,
        1,
    ...
    // ── Scenario 6 ───────────────────────────────────────────────────────────
    test(
      '6. Item with changed time string is updated in-place, not duplicated',
      () async {
        final (:container, :notifier) = await makeContainer();
        addTearDown(container.dispose);
        ...
        final aliceResult = reconciled.firstWhere(
          (m) => m.id == 'uuid-alice-1',
        );
    ...
    test('7. fetchFeed after init produces no duplicates', () async {
      ...
      expect(
        s.memories.map((m) => m.id).toSet().length,
        2,
