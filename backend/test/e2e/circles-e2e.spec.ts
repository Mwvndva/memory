...
describe('Circles E2E (sqlite)', () => {
  ...
  it('A sends request -> B accepts -> both see members and can message', async () => {
    ...
    const pending = await prisma.circleMembership.findFirst({
      where: { userId: A.id, memberId: B.id },
    });
    ...
    const aToB = await prisma.circleMembership
      .findUnique({
        where: { unique_user_member: { userId: A.id, memberId: B.id } },
      })
      .catch(() => null);
    const bToA = await prisma.circleMembership
      .findUnique({
        where: { unique_user_member: { userId: B.id, memberId: A.id } },
      })
      .catch(() => null);
    ...
    const wsA = new WebSocket(`ws://localhost:${port}/ws`, {
      headers: { Authorization: `Bearer ${tokenA_ws}` },
    });
    const wsB = new WebSocket(`ws://localhost:${port}/ws`, {
      headers: { Authorization: `Bearer ${tokenB_ws}` },
    });
    ...
    const payloadAtoB = {
      event: 'send_message',
      data: { receiver: B.username, text: 'Hello Bob' },
    };
    ...
    const payloadBtoA = {
      event: 'send_message',
      data: { receiver: A.username, text: 'Hi Alice' },
    };
    wsB.send(JSON.stringify(payloadBtoA));
    await new Promise((res) => setTimeout(res, 500));
    const msgsBA = await prisma.message.findMany({
      where: { senderId: B.id, receiverId: A.id },
    });
    ...
    const reqToC = await request(server)
      .post('/circles/requests')
      .set('authorization', `Bearer ${tokenA}`)
      .send({ memberId: C.id });
    ...
    // C declines
    const decline = await request(server)
    ...
    const aToC = await prisma.circleMembership
      .findUnique({
        where: { unique_user_member: { userId: A.id, memberId: C.id } },
      })
      .catch(() => null);
